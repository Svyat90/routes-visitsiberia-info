                ...
                <div class="row">
                        ...
                        <div class="tab-content">
                            @foreach($languages as $language)
                                <div class="tab-pane {{ $loop->index === 0 ? 'show active' : '' }}" role="tabpanel"
                                     id="{{ $language->locale }}">
                                    <div class="m-3">
                                        @foreach($route->getTranslatable() as $field)
                                            @php
                                                $oldLocale = old($field);
                                                ...
                                                @if (in_array($field, ['page_desc', 'history_desc', 'contact_desc']))
                                                    <div class="form-group">
                                                        ...
                                                        <textarea class="form-control tinymceTextarea {{ $errors->has($name) ? 'is-invalid' : '' }}"
                                                                  name="{{ $name }}" id="{{ $name }}">{!! $oldLocale[$language->locale] ?? $route->getTranslation($field, $language->locale) !!}</textarea>
                                                        @if($errors->has($name))
                                                            <span class="text-danger">{{ $errors->first($name) }}</span>
                                                        @endif
                                                        <span class="help-block">{{ __("cruds.routes.fields.{$field}_helper") }}</span>
                                                    </div>
                                                @else
                                                    <label class=""
                                                    ...
                                                    <input
                                                        ...
                                                        name="{{ $name }}"
                                                        id="{{ $name }}"
                                                        value="{{ $oldLocaleVal ?? $route->getTranslation($field, $language->locale) }}" />
                                                    @if($errors->has($name))
                                                        <span class="text-danger">{{ $errors->first($name) }}</span>
                                                    @endif
                                                    <span
                                                        class="help-block">{{ __("cruds.routes.fields.{$field}_helper") }}</span>
                                                @endif
                                            </div>
                                        @endforeach
                                    </div>
                    ...
                    <div class="form-group col-md-6 col-sm-6 col-xs-6">
                        <label class="required"
                               for="{{ $name = 'active' }}">{{ __("cruds.routes.fields.$name") }}</label>
                        <select name="{{ $name }}" id="{{ $name }}" class="form-control">
                            <option value="0" {{ old($name, $route->$name) == "0" ? 'selected' : '' }}>{{ __('global.no') }}</option>
                            <option value="1" {{ old($name, $route->$name) == "1" ? 'selected' : '' }}>{{ __('global.yes') }}</option>
                        </select>
                        @if($errors->has($name))
                            <span class="text-danger">{{ $errors->first($name) }}</span>
                        @endif
                        <span class="help-block">{{ __("cruds.routes.fields.{$name}_helper") }}</span>
                    </div>

                    <div class="form-group col-md-6 col-sm-6 col-xs-6">
                        <label class="required"
                               for="{{ $name = 'recommended' }}">{{ __("cruds.routes.fields.$name") }}</label>
                        <select name="{{ $name }}" id="{{ $name }}" class="form-control">
                            <option value="0" {{ old($name, $route->$name) == "0" ? 'selected' : '' }}>{{ __('global.no') }}</option>
                            <option value="1" {{ old($name, $route->$name) == "1" ? 'selected' : '' }}>{{ __('global.yes') }}</option>
                        </select>
                    ...
                    <div class="form-group col-md-6 col-sm-6 col-xs-6">
                        <label class="" for="{{ $name = 'site_link' }}">{{ __("cruds.routes.fields.$name") }}</label>
                    ...
                    <div class="form-group col-md-6 col-sm-6 col-xs-6">
                        <label class="" for="{{ $name = 'email' }}">{{ __("cruds.routes.fields.$name") }}</label>
                        <input class="form-control {{ $errors->has($name) ? 'is-invalid' : '' }}"
                               type="email"
                               name="{{ $name }}"
                               id="{{ $name }}"
                    ...
                    <div class="form-group col-md-6 col-sm-6 col-xs-6">
                        <label class="required" for="{{ $name = 'lat' }}">{{ __("cruds.routes.fields.$name") }}</label>
                        <input class="form-control {{ $errors->has($name) ? 'is-invalid' : '' }}"
                               type="text"
                               name="{{ $name }}"
                               id="{{ $name }}"
                    ...
                    <div class="form-group col-md-6 col-sm-6 col-xs-6">
                        <label class="required" for="{{ $name = 'lng' }}">{{ __("cruds.routes.fields.$name") }}</label>
                        <input class="form-control {{ $errors->has($name) ? 'is-invalid' : '' }}"
                               type="text"
                               name="{{ $name }}"
                               id="{{ $name }}"
                    ...
                    <div class="form-group col-md-12 col-sm-12 col-xs-12">
                        <label class="" for="{{ $name = 'image' }}">{{ __("cruds.routes.fields.$name") }}</label>
                    ...
                    <div class="form-group col-md-12 col-sm-12 col-xs-12">
                        <label class="" for="{{ $name = 'image_history' }}">{{ __("cruds.routes.fields.$name") }}</label>
                    ...
                    <div class="form-group col-md-12 col-sm-12 col-xs-12">
                        <label class="" for="{{ $name = 'image_gallery' }}">{{ __("cruds.routes.fields.$name") }}</label>
                        <div class="needsclick dropzone {{ $errors->has($name) ? 'is-invalid' : '' }}" id="{{ $name }}">
                        </div>
                        @if($errors->has($name))
                            <span class="text-danger">{{ $errors->first($name) }}</span>
                    ...
                    <div class="form-group col-md-12 col-sm-12 col-xs-12">
                        <label class="required" for="{{ $name = 'pdf_map' }}">{{ __("cruds.routes.fields.$name") }}</label>
                        <div class="needsclick dropzone {{ $errors->has('file') ? 'is-invalid' : '' }}" id="{{ $name }}">
                        </div>
                        @if($errors->has($name))
                            <span class="text-danger">{{ $errors->first($name) }}</span>
...
@section('scripts')
    <script>
        ...
        let imageDropZone = new Dropzone("#image", {
            ...
            init: function () {
                @php $media = $route->getFirstMedia($name = 'image'); @endphp
                @if($media)
        ...
        let pdfMapDropZone = new Dropzone("#pdf_map", {
            url: '{{ route('admin.media.upload') }}',
            maxFilesize: 50, // MB
            maxFiles: 5,
            addRemoveLinks: true,
            ...
            success: function (file, response) {
                $('form').find('input[name="pdf_map"]').remove()
            ...
            init: function () {
                @php $media = $route->getFirstMedia($name = 'pdf_map'); @endphp
                @if($media)
                var file = {!! json_encode($media) !!}
                    this.options.addedfile.call(this, file)
                this.options.thumbnail.call(this, file, '{{ $media->getUrl('thumb') }}')
        ...
        let imageHistoryDropZone = new Dropzone("#image_history", {
            ...
            init: function () {
                @php $media = $route->getFirstMedia($name = 'image_history'); @endphp
                @if($media)
                    var file = {!! json_encode($media) !!}
                        this.options.addedfile.call(this, file)
                    this.options.thumbnail.call(this, file, '{{ $media->getUrl('thumb') }}')
        ...
        let imageGalleryDropZone = new Dropzone("#image_gallery", {
            ...
            init: function () {
                @php $mediaList = $route->getMedia('image_gallery'); @endphp
                @if($mediaList)
